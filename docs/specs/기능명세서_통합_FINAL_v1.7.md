# 기능명세서 통합본 FINAL — 만세력/사주풀이/사주채팅/보관함/오행음악/n8n

- 버전: v1.7
- 최종 업데이트: 2025-12-21 (Asia/Seoul)
- 상태: FINAL

## 이번 통합본(v1.7)에 포함된 “패치 흡수” 항목
- 만세력: 시간 모름(null) 처리, 새로고침/임시보관 UX, ephe 파일 누락 방지, 한국 DST/역사적 TZ 주의
- 사주풀이: Lock A(차트 삭제 금지/RESTRICT), Lock B(리포트 생성 멱등/복구), 리포트 스냅샷 불변 + 재생성 버튼
- 사주채팅: Next.js Gateway 강제, 크레딧 장부(credit_logs), RPC 원자화(save_assistant_and_deduct), 캐시/요약/비상모드(text_only) + JSON repair + 서킷브레이커
- 공통: Legal/SEO/Sentry/Support, rate limit, request_id 로그, 백업/마이그레이션 원칙
- 운영 준비물: `npm run seed` + `.env.example`

## 공통 운영 요구사항(강제)
- 본 문서 하단 부록(NFR/DB/Seed)을 개발자가 그대로 적용해야 “배포 가능” 상태로 인정한다.

---

- 작성일: 2025-12-21
- 범위: 만세력(Phase1) / 사주풀이(Phase2) / 사주채팅·상담(Phase3) / /vault / n8n 운영 규칙

## 0. 공통 정책 Lock
- 유료 콘텐츠 프론트 숨김 금지 → **Next.js 서버 필터링**
- 상담 단독 판매 없음 → `사주풀이+상담 패키지` + `추가 상담 결제`
- 상담은 **사주풀이 리포트 이후에만**
...
## 1. 라우팅
- Public: `/`, `/mansaeryeok`, `/mansaeryeok/result`, `/pricing`, `/reviews`, `/music`
- Auth: `/vault`, `/vault/charts`, `/vault/reports`, `/vault/chats`
- Account: `/account/profile`, `/account/billing`, `/account/credits`
...
## 3. Phase2 사주풀이(유료)
- 상품: `사주풀이 단독` / `사주풀이+상담 패키지`
- 결제는 **로그인 직후**에만 노출(최종확인 후 결제 금지)
- 유료 사주풀이용 사주 입력: **1개 확정 + 잠금(변경 불가)**
- 권한 확정 처리(원자): entitlement에 `bound_chart_id` 기록 + chart 잠금 트랜잭션
...
## 5. n8n 강제 규칙(비용/캐시/요약)
### 5.1 예산
- DB 원문 발췌: 최대 2000자
- chart_facts_pack: 600~900자(항상 포함)
- summary: 600~700자
- 응답: 700~1200자(필요시 1800자)

### 5.2 캐시(수정)
- intent 의존 제거 → **topic_bucket(primary/secondary)** 사용
- 우선순위: timing > money > work > love > family > health > general
- 2단 캐시:
  - L1: owner + chart + topic_primary + message_hash
  - L2: owner + chart + topic_primary 후보 중 룰 기반 유사도 선택

### 5.3 요약(수정)
- 트리거: 누적 1500자 OR 6턴 OR 핵심 이벤트
- JSON: Repair → Validate → Fallback
- Fallback 3회 연속이면 `text_only`로 전환 + Warning 로그
- text_only 규격(600자):
  1) 사주 핵심 팩트 2줄 2) 주제 1줄 3) 목표 1줄 4) 실행 2줄 5) 미해결 1줄
- 프롬프트에는 요약 **하나만**(structured/text_only 동시 금지)
- 요약 null 처리: **완전한 빈 문자열 ''** (공백 금지) → Code Node 강제

## 6. DB 필수 제약
- `chat_messages(owner_id, idempotency_key)` unique
- 저장+차감은 1 RPC로 원자 처리(분리 금지)
---

# ✅ [추가 반영] Phase2 사주풀이 감리 패치(지뢰 3종) — 기능명세서 v1.2

## P2-LOCK-01 잠김 차트 삭제 금지
- 조건: `chart.is_locked=true`
- 허용: 숨김 처리만(`is_hidden=true` 또는 `hidden_at`)
- 금지: 삭제(DELETE), soft-delete(deleted_at)도 금지
- 응답: 403 + 고정 메시지

## P2-LOCK-02 유료 리포트 스냅샷 불변성
- 기본 열람: `snapshot_payload_json` 그대로 렌더
- 금지: upsert 덮어쓰기
- 재생성: 새 리비전 생성(혹은 새 report row)
- 버전 기록: `engine_version`, `content_version`, `revision`

## P2-LOCK-03 Bound 후 생성 실패 복구
- 상태: entitlement.bound_chart_id 존재 AND report 없음
- UX: 자동 재시도 1회 → 실패 시 `리포트 생성(무료)` 버튼
- 서버: `create_report_if_missing(entitlement_id)` 멱등 API/RPC
- 금지: 결제/차감 루트 재진입

## DB 패치(권장 SQL 방향)
- BEFORE DELETE trigger on `saju_charts` to block when `is_locked=true`
- UNIQUE: `saju_reports(entitlement_id, revision)` 또는 `saju_reports(entitlement_id)`(단일 스냅샷)  
  + 재생성은 별도 `saju_report_revisions` 테이블도 가능
- `report_status`, `report_generation_attempts`, `report_last_error` 추가(세션/권한 테이블에 넣어도 됨)
---

# ✅ [추가 반영] Phase1 만세력 감리 패치(지뢰 3종) — 기능명세서 v1.3

## P1-EX-01 시간 모름(Unknown Time)
- 입력: `birthTime=null`, `timeAccuracy="unknown"`
- 서버: `hourPillar=null` 반환(임의 00:00 보정 금지)
- UI: 시주 영역 `시간 모름/미상(未詳)` 표기
- 파생값: 시주 의존 항목은 “시간 미상으로 제외” 표시

## P1-EX-02 게스트 새로고침(F5) 내구성
- 게스트 결과/입력은 sessionStorage에 저장
- 결과 페이지는 mount 시 sessionStorage로 복구 렌더(React state 단독 금지)
- 임시 안내문 유지:
  - “지금은 임시 결과입니다. 브라우저를 닫으면 사라져요. [보관함에 영구 저장하기]”

## P1-DEP-01 배포 파일 누락 방지(ephe)
- Ephemeris 파일은 `public/ephe`에 포함
- 런타임 경로는 `process.cwd()` 기준
- CI: `prebuild`에서 verify script로 누락 시 빌드 실패
---

# ✅ [추가 반영] Phase1 역사적 타임존/DST 강제 — 기능명세서 v1.4

## P1-TZ-01 한국 DST/표준시 변동 반영(Asia/Seoul Historical TZ)
- 금지: `new Date("...")` 등 Date 객체로 로컬 시간을 바로 처리
- 필수: IANA TZ("Asia/Seoul") 기준으로 UTC 변환 후 엔진에 투입
- 구현: luxon 또는 date-fns-tz 사용
- 운영: 배포 환경 tzdata/ICU 포함 확인(특히 컨테이너)
---

# ✅ [추가 반영] Phase1 DST 전환 시각 방어 + 자동 테스트 — 기능명세서 v1.5

## P1-TZ-02 DST 전환 시각(존재하지 않음/모호함) 처리
- DST 시작일 02:00~02:59 입력은 **422로 거절** (자동 보정 금지)
- DST 종료일 02:00~02:59는 **모호함 감지**
  - 정책 A(권장): UI에서 dstPreference 선택 후 확정
  - 정책 B: 표준시 우선(단, 분쟁 리스크)
- 자동 검증 벡터 5종(V1~V5)로 런타임 환경 검증
---

# ✅ [재확인/강화 반영] Phase2 유료 사주풀이 — Lock A / Lock B (기능명세서 v1.6)

> 목적: PRD2에 반영된 Lock A/Lock B가 **기능명세서에도 빠짐 없이 고정**되도록, 구현/QA 관점으로 재정의한다.  
> (본 섹션은 기존 명세를 덮어쓰지 않는 append-only이며, 충돌 시 본 섹션 우선)

## P2-LOCKA-01 유료 리포트 연결 차트 삭제 방지(DB 강제)
- 대상 테이블: `saju_charts`, `interpretation_reports`
- 강제 제약:
  - `interpretation_reports.chart_id` → `saju_charts.id`
  - FK 옵션: **ON DELETE RESTRICT** (CASCADE 금지)
- 강제 정책:
  - `saju_charts.is_locked=true` 인 차트는 **삭제 불가**
  - 정리 기능은 삭제가 아니라 **숨김 처리만 허용**(예: `is_hidden=true` 또는 `hidden_at`)

### 서버/API 규격
- 차트 삭제 시도 시:
  - 응답 코드: **403 또는 409 중 1개로 통일**(권장 403)
  - 메시지(고정): `삭제 불가: 숨김 처리만 가능합니다.`
- 프론트:
  - 삭제 버튼은 locked 차트에서 비활성/숨김
  - “숨김” 기능으로 대체

### QA 체크
1) locked 차트 삭제 요청 → 403/409 + 고정 메시지
2) report가 있는 차트 삭제 요청(FK restrict) → 삭제 불가

---

## P2-LOCKB-01 리포트 생성 멱등성 + 원자성(스마트 자판기)
- 대상 API: `POST /api/interpretation/report`
- 원칙: “100번 눌러도 결제/차감은 1번, 리포트는 1개(또는 최신 revision 1개)”

### 상태 정의
- Unbound: entitlement에 `bound_chart_id` 없음
- Bound: entitlement에 `bound_chart_id` 있음
- ReportExists: 해당 entitlement(및 revision 정책)에 해당하는 report row 존재

### 강제 로직(순서 고정)
1) entitlement 조회 + owner 검증
2) 분기:

**(A) Bound && ReportExists**
- 기존 report **그대로 반환**
- 결제/차감/바인딩/잠금 로직 금지

**(B) Bound && ReportMissing**
- 차감 없이 **리포트 생성만 재시도**
- 동시 요청에도 안전해야 함(멱등 생성)

**(C) Unbound**
- 아래 3개를 **하나의 트랜잭션**으로 처리(부분 성공 금지):
  1) entitlement bound 처리(`bound_chart_id=chart_id`)
...
- 최소:
  - `create_report_if_missing(entitlement_id)` 멱등 RPC + bound/lock 트랜잭션

### 프론트 UX 강제
- 리포트 페이지 진입 시 report missing 감지:
  - 자동 재시도 1회
...
### QA 체크
1) report 생성 API 10회 연속 호출 → report 1개/차감 1회
2) bound 직후 서버 다운(report missing) → 재호출로 무료 복구
3) bound chart와 다른 chart_id로 재요청 → 409/422로 차단

---

## P2-SNAPSHOT-01 리포트 스냅샷 불변성(업서트 금지)
- 유료 리포트는 생성 시점 결과를 `payload_json`으로 저장하고 자동 갱신 금지
- 로직 업데이트/알고리즘 변경 시에도 기존 구매 리포트 내용은 유지
- 재생성은:
  - 명시적 사용자 행동([재생성])으로만
  - 덮어쓰기(upsert) 대신 **새 revision 생성**(권장)
