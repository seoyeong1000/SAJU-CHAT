...
## 통합 패치 반영 메모
- Lock A/Lock B/스냅샷 불변/재생성 복구 UI까지 포함
- 차트/권한/리포트 관계는 DB Source of Truth 부록 기준

- 작성일: 2025-12-21
- 판매 구조(강제): “사주풀이 단독” 또는 “사주풀이+상담 패키지”
- 전제(강제): 채팅 상담은 **리포트 생성 이후**에만 가능

---

## 1. 모델(핵심)
- chart: 만세력 결과 1개
- entitlement: 결제 1건(=1개) 단위 권한
- report: 유료 결과물(스냅샷)

권장 연결:
- entitlement.bound_chart_id (차트 고정)
- report.entitlement_id, report.chart_id

---

## 2. 유료 차트 확정(강제)
- 유료 사주풀이에 사용할 차트는 **1개만 확정**
- 절차: 차트 선택/신규입력 → 최종확인 → 확정(=권한 확정 처리)

---

## 3. Lock A / Lock B / Snapshot (감리 강제)
### 3.1 Lock A(삭제 방지)
- FK: report.chart_id → saju_charts.id
- **ON DELETE RESTRICT** (CASCADE 금지)
- is_locked=true 차트는 삭제 불가(숨김만)

### 3.2 Lock B(멱등/복구)
- POST `/api/interpretation/report` 멱등 보장
- Bound+ReportExists: 기존 리포트 반환
- Bound+ReportMissing: 무료 재생성
- Unbound: (차감+잠금+생성) 트랜잭션 원자화

### 3.3 Snapshot 불변성
- payload_json은 구매 시점 스냅샷 유지
- 재생성은 사용자 명시 행동으로만(revision 권장)

---

## 4. “권한 확정 처리” = bound + lock 원자화(강제)
- entitlement에 chart_id 기록 + chart 잠금을 **한 트랜잭션**으로 처리
- 권장 RPC:
