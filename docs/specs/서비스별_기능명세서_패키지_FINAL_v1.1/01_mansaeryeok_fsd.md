...
## 통합 패치 반영 메모
- PRD1 패치(시간 모름, 새로고침 유지, ephe 파일 누락 방지, DST/역사적 TZ)까지 포함
- Seed/Env/NFR/DB 매핑 부록 포함

- 작성일: 2025-12-21
- 목표: 비로그인도 즉시 결과 확인(무료). 저장/보관함은 로그인 사용자만.
- 계산 엔진: `swisseph-wasm` (Node.js / Linux 배포 친화)

---

## 1. 사용자 플로우(핵심)
### 1.1 진입
- 메인(`/`) → [만세력] → 입력 폼(`/mansaeryeok/new`) → 결과(`/mansaeryeok/result`)

### 1.2 결과 열람 정책(강제)
- **본인**: 로그인 여부와 무관하게 결과 페이지는 표시 가능(단, 게스트는 “임시 결과”)
- **저장**: 로그인 사용자만 가능 → 저장 성공 시 보관함(`/vault`)에서 재열람 가능
- **공유**
  - 게스트 결과: 공유 버튼은 “우리 사이트 주소 공유”(홈/만세력 입력 링크)만 허용
  - 회원 저장 결과: 공유 링크 생성 가능(선택) — 공유 받은 사람은 로그인 필요(정책에 따라)

### 1.3 게스트 임시 결과(새로고침 방어)
- React state 단독 금지
- sessionStorage 저장/복구(같은 탭 F5 유지)
- 결과 상단 문구 고정:
...
- 생년월일 `birthDate` (필수)
- 출생시간 `birthTime` (옵션)
- “시간 모름” `timeAccuracy="unknown"` (옵션) → `birthTime=null`
- 출생지(선택): 시/도 + 구/군 또는 lat/lng
- 성별(선택)

### 2.2 시간 모름(Unknown Time) 강제 규칙
- timeAccuracy=unknown이면:
  - 서버에 `birthTime=null`
  - 서버는 시주 계산 금지 → `hourPillar=null`
  - UI는 “시간 모름/미상(未詳)” 표시
- 금지: 00:00(자시)로 채우기

### 2.3 DST/역사적 타임존(강제)
- `Asia/Seoul` IANA timezone 기반으로 **역사적 DST 반영** 후 UTC 변환 → 엔진 투입
- 금지: `Date` 직행, `+09:00` 고정
- 전환 시각 방어:
  - DST 시작일 02:00~02:59는 **422 거절**(자동 보정 금지)
  - DST 종료일 02:00~02:59는 **모호** → 정책 A(사용자 선택) 권장

---

## 3. 결과 화면(요구사항)
- 사주 8자(연/월/일/시) — 시간 모름이면 시주 비움
- 오행 분포/십신/기본 해석(무료 범위)
- 저장 CTA(게스트에 강조)
...
- POST `/api/mansaeryeok/save` (로그인 필수)

---

## 5. DB(개념)
- `saju_charts(id, owner_id nullable, input_json, result_json, is_locked, is_hidden, created_at, updated_at)`

---

## 6. QA 체크
- 시간 모름: 시주 null 유지 + UI 표기
- 게스트 F5: 결과 유지(sessionStorage)
- DST 벡터 테스트 통과
- 배포: `public/ephe/*.se1` 누락 시 빌드 실패(verify)
