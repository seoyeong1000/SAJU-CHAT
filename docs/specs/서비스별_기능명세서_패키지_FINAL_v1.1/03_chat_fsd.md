...
## 통합 패치 반영 메모
- Next.js Gateway 강제, 차감 원자화 RPC, 캐시/요약/서킷브레이커/JSON repair 포함
- 크레딧은 Ledger(credit_logs) 기준

- 작성일: 2025-12-21
- 판매 구조(강제): 패키지(리포트+상담) 또는 추가 상담(애드온)만

---

## 1. 전제 규칙(강제)
- 상담은 **리포트 생성 이후**에만 가능
- 클라→n8n 직결 금지(Next.js Gateway 필수)
- 차감: “assistant 저장 성공 1회 = 1차감”, 실패=0차감

---

## 2. 여러 결제/여러 사주 운영(강제)
- 상담 세션은 반드시 `report_id`(또는 base entitlement_id)에 바인딩
- 보관함에 리포트가 여러 개면 “상담할 리포트 선택”이 먼저

---

## 3. 저장/멱등(강제)
- chat_messages: `(owner_id, idempotency_key)` 유니크
- 저장+차감은 RPC 1개로 원자화:
  - `save_assistant_and_deduct(...)`

---

## 4. 요약/캐시(현실 보정)
- 캐시 키는 intent 의존을 줄이고:
  - `(report_id + normalized_hash + topic_bucket)` 권장
- topic_bucket 우선순위 강제(Money > Work > Love)
- 요약 트리거:
  - 10턴마다 + 누적 1500자 초과 시 즉시
- 요약 실패 3회 연속 → text_only 모드 강제
- text_only일 때 structured 요약은 프롬프트에서 제거(Null/'')

---

## 5. UX
- 남은 크레딧 표시(서버 응답값이 진실)
- 크레딧 0이면 추가 상담 결제로 유도(결제 성공 시 채팅으로 복귀)
