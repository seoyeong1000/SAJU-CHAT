...
## 통합 패치 반영 메모
- 각 서비스별 문서 + 공통 부록(NFR/DB/Seed) 포함 패키지

- 작성일: 2025-12-21
- 범위: 만세력(Phase1) / 사주풀이(Phase2) / 사주채팅(Phase3) / 보관함·마이페이지 / 오행음악 / n8n(운영·비용·캐시·요약)
- 원칙: 충돌 시 PRD의 “Policy Lock / Lock A / Lock B / 비용 규칙”이 최우선

## 문서 목록
1. `01_mansaeryeok_fsd.md` — 만세력(무료) + 저장/공유/시간모름/DST
2. `02_interpretation_fsd.md` — 유료 사주풀이(리포트) + Lock A/B + 스냅샷/재생성
3. `03_chat_fsd.md` — 사주채팅(리포트 기반) + 크레딧 + n8n 게이트웨이 + 차감 원자화
4. `04_vault_mypage_fsd.md` — /vault, 마이페이지, 회원관리, 결제 히스토리, 여러 사주/여러 결제 운영
5. `05_music_fsd.md` — 오행 음악(유튜브/임베드)
6. `06_n8n_fsd.md` — n8n 워크플로우(캐시/요약/토큰예산/DB 조회 제한/오류 복구)

## “여러 번 결제해서 여러 사주” 운영 핵심(결론)
- **결제 1건 = entitlement 1개**
- entitlement에 `bound_chart_id`(차트 고정)와 `bound_report_id`(리포트 연결)를 기록
- 추가 상담(애드온)은 `parent_entitlement_id`로 기존 entitlement에 매달아 동일 리포트/차트에 크레딧을 더한다
